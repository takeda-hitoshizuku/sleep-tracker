...
### 設定画面に睡眠症状・傾向のチェックリストを追加

- **設定タブをアコーディオン形式に変更**
  - 「睡眠の症状・傾向」と「活動時間帯」の2セクションをアコーディオンで管理
  - 1つ開いたら他は閉じるタイプ（排他開閉）
  - 今後の設定追加にも対応しやすい構造

- **症状チェックリストを追加**（4カテゴリ、計11症状）
  - 寝れない系：不眠症・入眠困難・中途覚醒・早朝覚醒
  - 起きれない系：過眠症・起立性調節障害・睡眠慣性
  - 不規則・昼夜逆転系：概日リズム睡眠障害・睡眠相後退症候群・交代勤務
  - その他：睡眠時無呼吸症候群・むずむず脚症候群
  - チェック状態は `localStorage` の `sleep-tracker-symptoms` に保存

- **AI分析プロンプトを症状に応じて動的に変更**
  - システムプロンプトに申告済み症状を反映し、症状の特性に沿った分析を指示
  - ユーザープロンプトにも症状リストを追加してコンテキストを強化
